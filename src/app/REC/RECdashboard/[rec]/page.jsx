        ...
        async function fetchData() {
            try {
                ...
                const filteredForms = response.data.forms.filter(form =>
                    form.researchEthicsCommittee && params.researchEthicsCommittee &&
                    form.researchEthicsCommittee.replace(/\s/g, '') === params.researchEthicsCommittee.replace(/\s/g, '')
                );
                        ...
                        <tbody>
                            {forms.map((form, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{form && form.date ? new Date(form.date).toLocaleDateString("en-US") : "No date available"}</td>
                                    <td>{form ? form.researchEthicsCommittee : "No REC available"}</td>
                                    <td
                                        className={`review-classification ${form ? form.reviewClassification : ""}`}
                                    >
