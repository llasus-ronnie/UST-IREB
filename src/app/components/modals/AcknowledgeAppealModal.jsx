    ...
    async function fetchData() {
      ...
      try {
        ...
        const matchingForm = response.data.forms.find(
          (form) => form._id === submissionparams
        );

        if (!matchingForm) {
          console.error(
            "No matching form found for the given submissionparams."
          );
          setForm(null);
          return;
        }
  ...
  return (
    ...
    >
      ...
      <Modal.Footer className="resubmission-modal-footer">
        <Button onClick={handleDecline} className="btn cancel">
          Decline
        </Button>
        <Button onClick={handleAccept} className="btn uploadproof">
          Accept
        </Button>
      </Modal.Footer>
      <ToastContainer />
